...
ANP Rescue is a repair and diagnostics environment. Treat client data and disks as safety-critical.
...
- The rescue client may perform bounded deterministic preflight checks, but it does not own model-backed agent reasoning. It calls the shared ANP backend through a narrow versioned API.
