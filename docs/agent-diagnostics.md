...
`anp-agent.exe` is the first local layer of the technician assistant. In this milestone it runs an offline deterministic preflight over `diagnostic-report.schema.json` and writes a `diagnosis.schema.json` assessment.

It is intentionally useful without internet access, but it is not presented as the final AI diagnosis. Model-backed reasoning, the repair knowledge base, device-token policy, rate limits, and audit storage remain in the shared ANP backend.

## Current flow

1. `anp-collector.exe` creates a structured read-only report.
2. `anp-agent.exe` validates the report version.
