...
| BitLocker | `Get-BitLockerVolume` | Falls back to `Win32_EncryptableVolume`. `bitlocker_inventory.status` is `ok`, `partial`, or `unavailable`. When unavailable, `bitlocker_volumes` is `null` rather than `[]`. Both `unavailable` and `partial` emit one `storage.bitlocker_inventory` unknown check (and therefore `evidence.sources-incomplete`). Nullable volume fields stay JSON `null`, never `""` |
